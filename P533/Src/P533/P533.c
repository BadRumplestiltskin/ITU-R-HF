...
/*
	BuildDataPath() - Joins a data directory and a file name into a bounded buffer.
		...
		buffer, which is unbounded on a long -d argument and left the separator
		as an undocumented obligation on the caller. This inserts the separator
		when the directory does not already end in one, and reports truncation
...
P533_API int BuildDataPath(char *out, size_t n, const char *dir, const char *file) {
	...
	if (out == NULL || n == 0 || dir == NULL || file == NULL) return FALSE;
	...
	int P533(struct PathData *path) {
		...
		P533()
			...
			OUTPUT
				...
				returns RTN_P533OK (10) on success, a ValidatePath() error code (100 - 125) for
					bad input, or the P372
