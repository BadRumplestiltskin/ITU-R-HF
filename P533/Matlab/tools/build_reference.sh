#!/bin/bash
# Build libp372.so and the P533 reference driver from the unmodified ITU C
# sources, then regenerate tests/reference/. See ../docs/VALIDATION.md.
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"; ROOT="$(cd "$HERE/.." && pwd)"
# Default to the checkout this script lives in: tools/ is P533/Matlab/tools.
ITU="${ITU_R_HF:-$(cd "$(dirname "$0")/../../.." && pwd)}"
P372="$ITU/P372/Src/P372"; P533="$ITU/P533/Src/P533"
DATA="$ITU/P533/Data/"; ANT="$ITU/ITURHFProp/Data/Antenna/T13 Files"
BUILD="$HERE/build"; REF="$ROOT/tests/reference"
mkdir -p "$BUILD" "$REF"
CFLAGS="-std=gnu11 -O2 -w -fcommon -include stdio.h -include stdlib.h -include string.h -include errno.h -include math.h -include time.h"
cp "$P533/Common.h" "$BUILD/common.h"
cd "$BUILD"
cc $CFLAGS -shared -fPIC -I"$P372" -I"$BUILD" "$P372/InitializeNoise.c" "$P372/MakeNoise.c" "$P372/Noise.c" "$P372/NoiseMemory.c" -o libp372.so -lm
cc $CFLAGS -I"$P533" -I"$BUILD" -o ref_p533 "$HERE/ref_p533.c" \
   "$P533/Between7000kmand9000km.c" "$P533/CalculateCPParameters.c" "$P533/CircuitReliability.c" \
   "$P533/ELayerScreeningFrequency.c" "$P533/Geometry.c" "$P533/InitializePath.c" "$P533/InputDump.c" \
   "$P533/MUFBasic.c" "$P533/MUFOperational.c" "$P533/MUFVariability.c" "$P533/Magfit.c" \
   "$P533/MedianAvailableReceiverPower.c" "$P533/MedianSkywaveFieldStrengthLong.c" \
   "$P533/MedianSkywaveFieldStrengthShort.c" "$P533/P533.c" "$P533/PathMemory.c" \
   "$P533/ReadIonParameters.c" "$P533/ReadP1239.c" "$P533/ReadType13.c" "$P533/ValidatePath.c" -ldl -lm
python3 "$HERE/make_cases.py" "$ANT" "$REF/cases.csv"
mkdir -p "$REF/dump"
./ref_p533 --dump "$DATA" "$ANT/141-10_0.t13" "$REF/dump/"
./ref_p533 "$DATA" "$REF/cases.csv" "$REF/ref_p533.csv"
ls -la "$REF" "$REF/dump"
